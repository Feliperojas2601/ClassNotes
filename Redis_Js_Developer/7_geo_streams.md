...
- **Notas:**
  - Redis geospatial coordinates permite guardar esa info geoespacial usando GeoHash y sorted sets. Todos los comandos tienen el prefix geo antes, como geoAdd o geoRadius que según coordenadas y un radio determina si hay registros geo dentro de este.
  - Streams en redis son una estructurar para sincronizar la data a otros sistemas. Cada entrada en un stream es id + fields/values, a manera de global log. Luego un consumidor se pega a ese log y es barato. Los comandos de los streams tienen el prefix X. Xadd y Xrange (registros de más viejos a nuevos) o XRevRange. En node, el sizing del stream es importante, la definición del maxlen es opcional pero igual existe.
- **Código:**
  ```javascript
